...
<xsl:stylesheet version="1.0" 
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='EmailNTJDelivery']" mode="content">
        <tr class="header">
        <td width="55" class="logo">
         ...
         <td class="title">
				...
				<xsl:apply-templates select="//EmailNTJDelivery.Titel"/>  
			</td>
      ...
      <tr>
        ...
        <td class="text-highlight">
        	<p>
        		<xsl:apply-templates select="//EmailNTJDelivery.Aanhef" />
        		<xsl:apply-templates select="//cas:property[cas:key='NaamContactpersoon']/cas:value" />,</p>
        	<p>
        		

				<xsl:apply-templates select="//EmailNTJDelivery.Afgerond"/>  
        	...
        	<p>
			 ...
			 <span class="highlight">Country code: 
        			<xsl:apply-templates select="//cas:property[cas:key='ReceivingCountry']/cas:value" mode="translate-value" />
        			<xsl:apply-templates select="//cas:property[cas:key='DoorgeeflandNaam']/cas:value" /></span><br />
        		

				<xsl:apply-templates select="//EmailNTJDelivery.TypeLevering"/>  

				 <span class="highlight">
        			<xsl:apply-templates select="//cas:property[cas:key='TypeLevering']/cas:value" mode="translate-value"/>
        			<xsl:text> </xsl:text>
					<xsl:apply-templates select="//EmailNTJDelivery.Levering"/>  
			...
			<p>
			<xsl:apply-templates select="//EmailNTJDelivery.Belastingjaar"/>  
				<span class="highlight">Tax year: 
        			<xsl:apply-templates select="//cas:property[cas:key='Belastingjaar']/cas:value" mode="translate-value" />
        		</span>
			</p>
			<br />

			<xsl:apply-templates select="//EmailNTJDelivery.OpenPortaalVoorResultaat"/>  
        	...
        	<p class="buttonlink">
        		<a>
        			...
        			&#32;&#32;&#32;&#32;&#32;&#32;
					<xsl:apply-templates select="//EmailNTJDelivery.OpenPortaal"/>  
					&#32;&#32;&#32;&#32;&#32;&#32;&#160;
        		</a></p>
        	<p>
			
			<xsl:apply-templates select="//EmailNTJDelivery.DocumentenOpgeslagen"/>  
			</p>
        </td>
        <td width="55" class="col3">&#32;</td>
      </tr>
    </xsl:template>
	<xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='EmailNTJDelivery']" mode="plaintext">
...
<xsl:apply-templates select="//EmailNTJDelivery.Verdrag"/>
<xsl:text xml:space="preserve">
      </xsl:text>
<xsl:apply-templates select="//cas:property[cas:key='Verdrag']/cas:value" />
<xsl:text xml:space="preserve">
      </xsl:text>
<xsl:apply-templates select="//EmailNTJDelivery.Jurisdictie"/>
<xsl:text xml:space="preserve">
      </xsl:text>
<xsl:apply-templates select="//cas:property[cas:key='DoorgeeflandNaam']/cas:value"  mode="translate-value"/>
<xsl:text xml:space="preserve">
      </xsl:text>
<xsl:apply-templates select="//cas:property[cas:key='DoorgeeflandNaam']/cas:value" />
<xsl:text xml:space="preserve">
      </xsl:text>
<xsl:apply-templates select="//EmailNTJDelivery.TypeLevering"/>
<xsl:text xml:space="preserve">
