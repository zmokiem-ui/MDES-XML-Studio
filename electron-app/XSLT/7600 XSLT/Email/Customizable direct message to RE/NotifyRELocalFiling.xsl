...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='NotifyRELocalFiling']" mode="content">
      ...
      <tr>
        ...
        <td class="text-highlight">
          <p><xsl:apply-templates select="//NotifyRELocalFiling.Aanhef"/></p>
          <p>
            <xsl:apply-templates select="//NotifyRELocalFiling.AanvraagVoorGebruik"/>
          </p>
          <p>
            <xsl:apply-templates select="//NotifyRELocalFiling.Tekst"/>
          </p>
        
        
          <p>
            <span class="highlight">
              <xsl:apply-templates select="//NotifyRELocalFiling.Belangrijk"/>
             </span>
             <xsl:apply-templates select="//NotifyRELocalFiling.Informatie"/>
          </p>
        </td>
        <td width="55" class="col3">&#32;</td>
      </tr>
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='NotifyRELocalFiling']" mode="plaintext">

      <xsl:apply-templates select="//NotifyRELocalFiling.Titel"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyRELocalFiling.Aanhef"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyRELocalFiling.AanvraagVoorGebruik"/>
<xsl:text xml:space="preserve">
...
<xsl:text xml:space="preserve">
      ...
      <xsl:apply-templates select="//NotifyRELocalFiling.Bevestig"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyRELocalFiling.LinkWerktNiet"/>
<xsl:text xml:space="preserve">
</xsl:text>
