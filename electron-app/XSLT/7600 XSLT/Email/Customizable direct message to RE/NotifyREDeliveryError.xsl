...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='NotifyREDeliveryError']" mode="content">
      ...
      <tr>
        ...
        <td class="text-highlight">
          <p><xsl:apply-templates select="//NotifyREDeliveryError.Aanhef"/></p>
          <p>
            <xsl:apply-templates select="//NotifyREDeliveryError.AanvraagVoorGebruik"/>
          </p>
          <p>
            <xsl:apply-templates select="//NotifyREDeliveryError.Tekst"/>
          </p>
        
        
          <p>
            <span class="highlight">
              <xsl:apply-templates select="//NotifyREDeliveryError.Belangrijk"/>
             </span>
             <xsl:apply-templates select="//NotifyREDeliveryError.Informatie"/>
          </p>
        </td>
        <td width="55" class="col3">&#32;</td>
      </tr>
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='NotifyREDeliveryError']" mode="plaintext">

      <xsl:apply-templates select="//NotifyREDeliveryError.Titel"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyREDeliveryError.Aanhef"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyREDeliveryError.AanvraagVoorGebruik"/>
<xsl:text xml:space="preserve">
...
<xsl:text xml:space="preserve">
      ...
      <xsl:apply-templates select="//NotifyREDeliveryError.Bevestig"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyREDeliveryError.LinkWerktNiet"/>
<xsl:text xml:space="preserve">
</xsl:text>
