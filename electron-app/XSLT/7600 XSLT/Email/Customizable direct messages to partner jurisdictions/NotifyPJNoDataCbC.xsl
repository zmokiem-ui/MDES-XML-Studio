...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='NotifyPJNoDataCbC']" mode="content">
        ...
        <td class="title"><xsl:apply-templates select="//NotifyPJNoDataCbC.Titel"/></td>
      ...
      <tr>
        ...
        <td class="text-highlight">
          <p><xsl:apply-templates select="//NotifyPJNoDataCbC.Aanhef"/></p>
          <p>
            <xsl:apply-templates select="//NotifyPJNoDataCbC.AanvraagVoorGebruik"/>
          </p>
          <p>
            <xsl:apply-templates select="//NotifyPJNoDataCbC.Tekst"/>
          </p>
        
        
          <p>
            <span class="highlight">
              <xsl:apply-templates select="//NotifyPJNoDataCbC.Belangrijk"/>
             </span>
             <xsl:apply-templates select="//NotifyPJNoDataCbC.Informatie"/>
          </p>
        </td>
        <td width="55" class="col3">&#32;</td>
      </tr>
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='NotifyPJNoDataCbC']" mode="plaintext">

      <xsl:apply-templates select="//NotifyPJNoDataCbC.Titel"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyPJNoDataCbC.Aanhef"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyPJNoDataCbC.AanvraagVoorGebruik"/>
<xsl:text xml:space="preserve">
...
<xsl:text xml:space="preserve">
      ...
      <xsl:apply-templates select="//NotifyPJNoDataCbC.Bevestig"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyPJNoDataCbC.LinkWerktNiet"/>
<xsl:text xml:space="preserve">
</xsl:text>
