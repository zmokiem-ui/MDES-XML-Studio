...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='RequestPJInformation']" mode="content">
        ...
        <td class="title"><xsl:apply-templates select="//RequestPJInformation.Titel"/></td>
      ...
      <tr>
        ...
        <td class="text-highlight">
          <p><xsl:apply-templates select="//RequestPJInformation.Aanhef"/></p>
          <p>
            <xsl:apply-templates select="//RequestPJInformation.AanvraagVoorGebruik"/>
          </p>
          <p>
            <xsl:apply-templates select="//RequestPJInformation.Tekst"/>
          </p>
        
        
          <p>
            <span class="highlight">
              <xsl:apply-templates select="//RequestPJInformation.Belangrijk"/>
             </span>
             <xsl:apply-templates select="//RequestPJInformation.Informatie"/>
          </p>
        </td>
        <td width="55" class="col3">&#32;</td>
      </tr>
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='RequestPJInformation']" mode="plaintext">

      <xsl:apply-templates select="//RequestPJInformation.Titel"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//RequestPJInformation.Aanhef"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//RequestPJInformation.AanvraagVoorGebruik"/>
<xsl:text xml:space="preserve">
...
<xsl:text xml:space="preserve">
      ...
      <xsl:apply-templates select="//RequestPJInformation.Bevestig"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//RequestPJInformation.LinkWerktNiet"/>
<xsl:text xml:space="preserve">
</xsl:text>
