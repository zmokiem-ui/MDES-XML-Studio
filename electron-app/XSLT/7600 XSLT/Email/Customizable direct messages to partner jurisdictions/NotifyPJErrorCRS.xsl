...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='NotifyRECbCReporting']" mode="content">
        ...
        <td class="title"><xsl:apply-templates select="//NotifyRECbCReporting.Titel"/></td>
      ...
      <tr>
        ...
        <td class="text-highlight">
          <p><xsl:apply-templates select="//NotifyRECbCReporting.Aanhef"/></p>
          <p>
            <xsl:apply-templates select="//NotifyRECbCReporting.AanvraagVoorGebruik"/>
          </p>
          <p>
            <xsl:apply-templates select="//NotifyRECbCReporting.Tekst"/>
          </p>
        
        
          <p>
            <span class="highlight">
              <xsl:apply-templates select="//NotifyRECbCReporting.Belangrijk"/>
             </span>
             <xsl:apply-templates select="//NotifyRECbCReporting.Informatie"/>
          </p>
        </td>
        <td width="55" class="col3">&#32;</td>
      </tr>
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='NotifyRECbCReporting']" mode="plaintext">

      <xsl:apply-templates select="//NotifyRECbCReporting.Titel"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyRECbCReporting.Aanhef"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyRECbCReporting.AanvraagVoorGebruik"/>
<xsl:text xml:space="preserve">
...
<xsl:text xml:space="preserve">
      ...
      <xsl:apply-templates select="//NotifyRECbCReporting.Bevestig"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyRECbCReporting.LinkWerktNiet"/>
<xsl:text xml:space="preserve">
</xsl:text>
