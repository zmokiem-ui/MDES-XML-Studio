...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='NotifyPJErrorCbC']" mode="content">
        ...
        <td class="title"><xsl:apply-templates select="//NotifyPJErrorCbC.Titel"/></td>
      ...
      <tr>
        ...
        <td class="text-highlight">
          <p><xsl:apply-templates select="//NotifyPJErrorCbC.Aanhef"/></p>
          <p>
            <xsl:apply-templates select="//NotifyPJErrorCbC.AanvraagVoorGebruik"/>
          </p>
          <p>
            <xsl:apply-templates select="//NotifyPJErrorCbC.Tekst"/>
          </p>
        
        
          <p>
            <span class="highlight">
              <xsl:apply-templates select="//NotifyPJErrorCbC.Belangrijk"/>
             </span>
             <xsl:apply-templates select="//NotifyPJErrorCbC.Informatie"/>
          </p>
        </td>
        <td width="55" class="col3">&#32;</td>
      </tr>
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='NotifyPJErrorCbC']" mode="plaintext">

      <xsl:apply-templates select="//NotifyPJErrorCbC.Titel"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyPJErrorCbC.Aanhef"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyPJErrorCbC.AanvraagVoorGebruik"/>
<xsl:text xml:space="preserve">
...
<xsl:text xml:space="preserve">
      ...
      <xsl:apply-templates select="//NotifyPJErrorCbC.Bevestig"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//NotifyPJErrorCbC.LinkWerktNiet"/>
<xsl:text xml:space="preserve">
</xsl:text>
