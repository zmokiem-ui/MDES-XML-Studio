...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='ActiveringscodeVersturen']" mode="content">
        ...
        <td class="title"><xsl:apply-templates select="//ActiveringscodeVersturen.Titel"/></td>
      ...
      <tr>
        ...
        <td class="text-highlight">
          <p><xsl:apply-templates select="//ActiveringscodeVersturen.Aanhef"/></p>
          <p>
            <xsl:apply-templates select="//ActiveringscodeVersturen.AanvraagVoorGebruik"/>
          </p>
          <p>
            <xsl:apply-templates select="//ActiveringscodeVersturen.KlikKnop"/>
          </p>
          <p class="buttonlink">
            <a>
              <xsl:attribute name="href"><xsl:apply-templates select="//cas:property[cas:key='Activatielink']/cas:value"/></xsl:attribute>
              &#32;&#32;&#32;&#32;&#32;&#32;<xsl:apply-templates select="//ActiveringscodeVersturen.Bevestig"/>&#32;&#32;&#32;&#32;&#32;&#32;&#160;</a
            >
          </p>
          <p>
          <xsl:apply-templates select="//ActiveringscodeVersturen.LinkWerktNiet"/>
            
            <span class="highlight"><xsl:value-of  select="//cas:property[cas:key='Activeringscode']/cas:value" /></span>
          </p>
          <p>
            <span class="highlight">
              <xsl:apply-templates select="//ActiveringscodeVersturen.Belangrijk"/>
             </span>
             <xsl:apply-templates select="//ActiveringscodeVersturen.Informatie"/>
          </p>
        </td>
        <td width="55" class="col3">&#32;</td>
      </tr>
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='ActiveringscodeVersturen']" mode="plaintext">

      <xsl:apply-templates select="//ActiveringscodeVersturen.Titel"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//ActiveringscodeVersturen.Aanhef"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//ActiveringscodeVersturen.AanvraagVoorGebruik"/>
<xsl:text xml:space="preserve">
...
<xsl:text xml:space="preserve">
      ...
      <xsl:apply-templates select="//ActiveringscodeVersturen.Bevestig"/>
<xsl:text xml:space="preserve">
</xsl:text>
      <xsl:apply-templates select="//ActiveringscodeVersturen.LinkWerktNiet"/>
<xsl:text xml:space="preserve">
