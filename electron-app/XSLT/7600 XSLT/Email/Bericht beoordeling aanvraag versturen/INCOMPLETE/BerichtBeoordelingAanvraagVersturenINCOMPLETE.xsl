...
<xsl:stylesheet version="1.0" 
    ...
    <xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='BerichtBeoordelingAanvraagVersturenINCOMPLETE']" mode="content">
        <tr class="header">
        ...
        <td class="title"><xsl:apply-templates select="//BerichtBeoordelingAanvraagVersturenINCOMPLETE.Titel"/></td>
      ...
      <tr>
        ...
        <td class="text-highlight">
          <p><xsl:apply-templates select="//BerichtBeoordelingAanvraagVersturenINCOMPLETE.Aanhef"/></p>
          <p>
            <xsl:apply-templates select="//BerichtBeoordelingAanvraagVersturenINCOMPLETE.AanvraagVoorGebruik"/>
          </p>
            <p>
	            <xsl:apply-templates  select="//BerichtBeoordelingAanvraagVersturenINCOMPLETE.AcknowledgementNumber" /> :
	          	<xsl:apply-templates  select="//cas:property[cas:key='originelecaseid']/cas:value" /><br />
            </p> 
          <p>
            <xsl:apply-templates select="//BerichtBeoordelingAanvraagVersturenINCOMPLETE.DientInTeLoggen"/>
          </p>
          <p class="buttonlink">
            <a>
            <xsl:attribute name="href"><xsl:apply-templates select="//cas:property[cas:key='PortaalLoginLink']/cas:value"/></xsl:attribute>
            &#32;&#32;&#32;&#32;&#32;&#32;<xsl:apply-templates select="//BerichtBeoordelingAanvraagVersturenINCOMPLETE.OpenPortaal"/>&#32;&#32;&#32;&#32;&#32;&#32;&#160;</a
            >
          </p>
          <p>
            <span class="highlight">
              <xsl:apply-templates select="//BerichtBeoordelingAanvraagVersturenINCOMPLETE.Toelichting"/>
            </span
            >
          </p>
          <p><xsl:apply-templates  select="//cas:property[cas:key='RedenAfwijzenAanvraag']/cas:value" /></p>
        </td>
        <td width="55" class="col3">&#32;</td>
      </tr>
    </xsl:template>
<xsl:template match="*[//cas:property[cas:key='MailIdentifier']/cas:value='BerichtBeoordelingAanvraagVersturenINCOMPLETE']" mode="plaintext">


    <xsl:text xml:space="preserve">
    </xsl:text>
  <xsl:apply-templates select="//BerichtBeoordelingAanvraagVersturenINCOMPLETE.Titel"/>
    <xsl:text xml:space="preserve">
    </xsl:text>
  <xsl:apply-templates select="//BerichtBeoordelingAanvraagVersturenINCOMPLETE.Aanhef"/>
    <xsl:text xml:space="preserve">
    </xsl:text>
  <xsl:apply-templates select="//BerichtBeoordelingAanvraagVersturenINCOMPLETE.AanvraagVoorGebruik"/>
    <xsl:text xml:space="preserve">
  ...
  <xsl:apply-templates select="//BerichtBeoordelingAanvraagVersturenINCOMPLETE.OpenPortaal"/>
    <xsl:text xml:space="preserve">
    </xsl:text>
  <xsl:apply-templates select="//BerichtBeoordelingAanvraagVersturenINCOMPLETE.Toelichting"/>
    <xsl:text xml:space="preserve">
    </xsl:text>
  <xsl:apply-templates  select="//cas:property[cas:key='RedenAfwijzenAanvraag']/cas:value" />
    <xsl:text xml:space="preserve">
    </xsl:text>
</xsl:template>
    </xsl:stylesheet>