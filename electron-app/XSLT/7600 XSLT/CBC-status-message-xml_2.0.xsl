...
exclude-result-prefixes="bi knowledge cmf case form report search assistant today dataeditor attributes usermanagement subscriptionmanagement organisationmanagement serviceapplication">
	...
	<xsl:template match="attributes:attributeset[contains(attributes:label,$xml-root-prefix)]">
		...
		<stm:CbCStatusMessage_OECD xsi:schemaLocation="urn:oecd:ties:cbcstm:v2 CBCStatusMessageXML_v2.0.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:stm="urn:oecd:ties:cbcstm:v2" xmlns:iso="urn:oecd:ties:cbcstm:isostmtypes:v1" version="2.0">			
	...
	<xsl:template name="resolve-namespace-uri">
		...
		<xsl:choose>
			<xsl:when test="$namespace-prefix='stm'">urn:oecd:ties:cbcstm:v2</xsl:when>
