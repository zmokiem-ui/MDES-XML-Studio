...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="default-namespace" select="'urn:oecd:ties:cbc:v2'"/>
    <xsl:variable name="default-prefix" select="'cbc:'"/>
    <xsl:include href="cbc-xml-genericv2.xsl"/>
    ...
    <xsl:template match="/bi:document-data">
        <!-- xsi:schemaLocation="urn:oecd:ties:cbc:v2 file:/Users/oneton/Documents/BearingPoint/MDES/XSD/CbCXML_v1.0.1.xsd" -->
        <cbc:CBC_OECD>
    ...
    <xsl:template match="attributes:attributeset[attributes:label='Message']/attributes:attributes">
        ...
        <xsl:element name="{concat($default-prefix,'MessageSpec')}">
    ...
    <xsl:template match="attributes:attributeset[attributes:label='Reporting entity']/attributes:attributes">
        <xsl:element name="{concat($default-prefix,'Entity')}">
            <xsl:apply-templates select="*[1]"/>
            <xsl:apply-templates select="attributes:label-attribute"/>
        </xsl:element>
        
        <xsl:apply-templates select="attributes:readonly-attribute[attributes:id='NameMNEGroup']"/>
        	
        <xsl:element name="{concat($default-prefix,'ReportingPeriod')}">
        	<xsl:element name="{concat($default-prefix,'StartDate')}">
            <xsl:apply-templates select="attributes:readonly-attribute[attributes:id='StartDate']//attributes:value" mode="not-empty"/>
            </xsl:element>
            <xsl:element name="{concat($default-prefix,'EndDate')}">
            <xsl:apply-templates select="attributes:readonly-attribute[attributes:id='EndDate']//attributes:value"  mode="not-empty"/>
            </xsl:element>
    ...
    <xsl:template match="attributes:value" mode="not-empty">
    	<xsl:if test=".=''">
    ...
    <xsl:template match="attributes:attributeset[attributes:label='Report']/attributes:attributes">
        ...
        <xsl:element name="{concat($default-prefix,'Summary')}">
            <xsl:element name="{concat($default-prefix,'Revenues')}">
    ...
    <xsl:template match="attributes:attributeset[attributes:label='Constituent entity']/attributes:attributes">
        <xsl:element name="{concat($default-prefix,'ConstEntity')}">
            <xsl:apply-templates select="*[1]"/>
            <xsl:apply-templates select="attributes:label-attribute"/>
        </xsl:element>
        <xsl:apply-templates select="attributes:readonly-attribute[attributes:id='Role']"/>
    </xsl:template>
    ...
    <xsl:template match="attributes:attributes" mode="docspec">
        <xsl:element name="{concat($default-prefix,'DocSpec')}">
    ...
    <xsl:template match="attributes:attributeset[attributes:layout-hint/attributes:hint='address']/attributes:attributes">
        ...
        <xsl:if test="attributes:readonly-attribute[attributes:layout-hint/attributes:hint='addressfix' and string(attributes:value)]">
            <xsl:element name="{concat($default-prefix,'AddressFix')}">
    ...
    <xsl:template match="attributes:label-attribute[attributes:layout-hint/attributes:hint='ref']">
        <xsl:choose>
            ...
            <xsl:when test="attributes:label//bi:resolved-message='Address'">
                ...
                <xsl:if test="ancestor::bi:attribute-set-document-part[bi:label='Constituent entity']//attributes:readonly-attribute[attributes:id='ConstituentEntityRecordId']/attributes:value!=''">  
