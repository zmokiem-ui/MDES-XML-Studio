...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="/">
		<cas:eventResponse>
			<xsl:apply-templates select="//cas:eventResponse"/>
		</cas:eventResponse>
	</xsl:template>
	
	<xsl:template match="cas:eventResponse">
	...
	<xsl:template match="*">
		<xsl:variable name="path" select=".//cas:property[cas:key='path']/cas:value"/>
		<xsl:copy-of select="." />
		<xsl:copy-of select="//cas:dataset[.//cas:property[cas:key='ContextPath']/cas:value=$path]"/>
	</xsl:template>
