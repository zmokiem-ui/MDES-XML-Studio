...
<xsl:stylesheet version="2.0" 
	...
	<xsl:import href="cbc-validation-templates.xsl"/>
	...
	<xsl:variable name="MessageRefId" select="*/*[local-name()='MessageSpec']/*[local-name()='MessageRefId']"/>
	<xsl:variable name="TransmittingCountry" select="*/*[local-name()='MessageSpec']/*[local-name()='TransmittingCountry']"/>
	<xsl:variable name="ReceivingCountry" select="*/*[local-name()='MessageSpec']/*[local-name()='ReceivingCountry']"/>
	<xsl:variable name="ReportingPeriod" select="*/*[local-name()='MessageSpec']/*[local-name()='ReportingPeriod']"/>
	<xsl:variable name="MessageTypeIndic" select="*/*[local-name()='MessageSpec']/*[local-name()='MessageTypeIndic']"/>
	...
	<xsl:template match="/">
		<cas:eventResponse>
			<cas:caseId />
	   		<xsl:apply-templates select="//*[local-name()='MessageSpec']" />
	   		<xsl:apply-templates select="//*[local-name()='CbcBody']" />	   		
		</cas:eventResponse>		
	</xsl:template>
