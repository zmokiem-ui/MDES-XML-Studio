...
exclude-result-prefixes="bi knowledge cmf cas case form report search assistant today dataeditor attributes usermanagement subscriptionmanagement organisationmanagement serviceapplication">
	<xsl:param name="mode"/>
	<xsl:param name="iterator"/>
	<xsl:variable name="MessageTypeIndic" select="//crs:MessageSpec/crs:MessageTypeIndic"/>
	<xsl:template match="/">
			<xsl:choose>
				<xsl:when test="$mode='clean'">
					<xsl:choose>
				...
				<xsl:otherwise>	
					<xsl:choose>
						...
						<crs:CRS_OECD xmlns:crs="urn:oecd:ties:crs:v2" xmlns:cfc="urn:oecd:ties:commontypesfatcacrs:v2" xmlns:iso="urn:oecd:ties:isocrstypes:v1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"  xmlns:stf="urn:oecd:ties:crsstf:v5" xsi:schemaLocation="urn:oecd:ties:crs:v2 CrsXML_v2.0.xsd" version="2.0">
							<xsl:apply-templates select="/*"  mode="clean-inverted"/>
						</crs:CRS_OECD>
						</xsl:when>
						<xsl:otherwise>
							<ftc:FATCA_OECD xmlns:ftc="urn:oecd:ties:fatca:v2" xmlns:stm="urn:oecd:ties:cbcstm:v2" xmlns:oecd_ntj="urn:oecd:ties:ntj:v1" xmlns:iso="urn:oecd:ties:isocrstypes:v1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sfa="urn:oecd:ties:stffatcatypes:v2" xsi:schemaLocation="urn:oecd:ties:fatca:v2 FatcaXML_v2.0.xsd" version="2.0">
								<xsl:apply-templates select="/*"  mode="clean-inverted"/>
							</ftc:FATCA_OECD>
						</xsl:otherwise>		
						</xsl:choose>
	...
	<xsl:template match="crs:MessageSpec" mode="clean">
		<crs:MessageSpec>
	...
	<xsl:template match="crs:MessageRefId" mode="header">
		<crs:MessageRefId><xsl:value-of select="concat(.,'.',$iterator)"/></crs:MessageRefId>
	</xsl:template>
		
	<xsl:template match="ftc:MessageSpec" mode="clean">
		<xsl:copy-of select="."/>
	
	</xsl:template>
	
	<xsl:template match="crs:CrsBody" mode="clean">
		<xsl:if test="count(crs:ReportingGroup/*) &gt; 0">
			<crs:CrsBody>
				<xsl:copy-of select="crs:ReportingFI"/>
				<crs:ReportingGroup>
					<xsl:apply-templates select="crs:ReportingGroup/*" mode="clean-accountreports"/>
				</crs:ReportingGroup>			
			</crs:CrsBody>	
		</xsl:if>
	</xsl:template>
	
	<xsl:template match="*" mode="clean-accountreports">
		<xsl:variable name="CorrDocRefId" select="crs:DocSpec/stf:CorrDocRefId"/>
		<xsl:variable name="numberOfCorrespondingCorrDocRefids" select="count(../*[crs:DocSpec/stf:DocRefId=$CorrDocRefId])"/>
		<xsl:if test="$numberOfCorrespondingCorrDocRefids=0">
			<xsl:copy-of select="." />
		</xsl:if>
	</xsl:template>
	
	<xsl:template match="crs:CrsBody" mode="clean-inverted">
		
		
		<xsl:if test="count(crs:ReportingGroup/*) &gt; 0">
			<xsl:variable name="accountReportNodes">
				<xsl:apply-templates select="crs:ReportingGroup/*" mode="clean-inverted-accountreports"/>
			</xsl:variable>
			 <xsl:if test="$accountReportNodes != ''">
			<crs:CrsBody>
				<xsl:copy-of select="crs:ReportingFI"/>
				<crs:ReportingGroup>
					<xsl:apply-templates select="crs:ReportingGroup/*" mode="clean-inverted-accountreports"/>	
	...
	<xsl:template match="*" mode="clean-inverted-accountreports">
		<xsl:variable name="CorrDocRefId" select="crs:DocSpec/stf:CorrDocRefId"/>
		<xsl:variable name="numberOfCorrespondingCorrDocRefids" select="count(../*[crs:DocSpec/stf:DocRefId=$CorrDocRefId])"/>
		<xsl:if test="$numberOfCorrespondingCorrDocRefids &gt; 0">
			<xsl:copy-of select="." />
	...
	<xsl:template match="ftc:FATCA" mode="clean">
		
		<xsl:variable name="CorrDocRefId" select="ftc:ReportingFI/ftc:DocSpec/ftc:CorrDocRefId"/>
		<xsl:variable name="numberPrecedingDuplicates" select="count(preceding-sibling::*/ftc:ReportingFI[ftc:DocSpec/ftc:DocRefId=$CorrDocRefId])"/>
		<xsl:variable name="numberFollowingDuplicates" select="count(following-sibling::*/ftc:ReportingFI[ftc:DocSpec/ftc:DocRefId=$CorrDocRefId])"/>
		<xsl:if test="$numberPrecedingDuplicates=0">
			<xsl:copy-of select="."/>
		</xsl:if>
	...
	<xsl:template match="*" mode="clean-inverted">
	
		<xsl:apply-templates  mode="clean-inverted"/>
	
	</xsl:template>
	<xsl:template match="crs:MessageSpec" mode="clean-inverted">
		<xsl:copy-of select="."/>
	
	</xsl:template>
	
	<xsl:template match="ftc:MessageSpec" mode="clean-inverted">
		<xsl:copy-of select="."/>
	...
	<xsl:template match="ftc:FATCA" mode="clean-inverted">
		<xsl:variable name="CorrDocRefId" select="ftc:ReportingFI/ftc:DocSpec/ftc:CorrDocRefId"/>
		<xsl:variable name="numberPrecedingDuplicates" select="count(preceding-sibling::*/ftc:ReportingFI[ftc:DocSpec/ftc:DocRefId=$CorrDocRefId])"/>
		<xsl:variable name="numberFollowingDuplicates" select="count(following-sibling::*/ftc:ReportingFI[ftc:DocSpec/ftc:DocRefId=$CorrDocRefId])"/>
		<xsl:if test="$numberPrecedingDuplicates > 0">
	...
	<xsl:template match="crs:CrsBody" mode="add-duplicate-accountreports">
		...
		<crs:ReportingGroup>
			...
			<xsl:copy-of select="preceding-sibling::*[crs:ReportingFI[crs:IN=$IN]]/crs:ReportingGroup/*"/>
		</crs:ReportingGroup>
		</crs:CrsBody>
	</xsl:template>
	...
	<xsl:template match="ftc:FATCA" mode="add-duplicate-accountreports">
		...
		<ftc:ReportingGroup>
			...
			<xsl:copy-of select="preceding-sibling::*[ftc:ReportingFI[sfa:TIN=$TIN]]/ftc:ReportingGroup/*"/>
