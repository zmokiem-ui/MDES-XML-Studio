...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="attributes:attributeset[contains(attributes:label,$xml-root-prefix)]">
		...
		<ftc:FATCA_OECD xsi:schemaLocation="urn:oecd:ties:fatca:v2 FatcaXML_v2.0.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:ftc="urn:oecd:ties:fatca:v2" xmlns:sfa="urn:oecd:ties:stffatcatypes:v2">
	...
	<xsl:template name="resolve-namespace-uri">
		...
		<xsl:choose>
			<xsl:when test="$namespace-prefix='ftc'">urn:oecd:ties:fatca:v2</xsl:when>
			<xsl:when test="$namespace-prefix='sfa'">urn:oecd:ties:stffatcatypes:v2</xsl:when>
