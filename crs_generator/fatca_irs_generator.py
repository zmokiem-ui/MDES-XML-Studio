...
class FATCAGenerator:
    ...
    def _add_substantial_owners(self, account: etree._Element, ns: dict):
        ...
        SubstantialOwner sits between AccountHolder and AccountBalance in
        CorrectableAccountReport_Type, so insert before AccountBalance rather
        than appending (which would land after AccountBalance/Payment).
        """
        ...
        for i in range(self.config.substantial_owners_per_org):
            so = etree.Element(f"{{{self.ns['ftc']}}}SubstantialOwner")
            parent.insert(balance_index + i, so)
    ...
    def generate(self) -> Path:
        ...
        if self.config.individual_accounts_per_fi + self.config.organisation_accounts_per_fi == 0:
            raise ValueError(
                "Cannot generate a FATCA (FATCA_OECD) message with zero accounts: "
                "the schema requires at least one AccountReport. Set "
                "--individual-accounts and/or --organisation-accounts."
            )
